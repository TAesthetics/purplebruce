...
//  Full Autonomy + Root Access + Full Audit & Playbooks
//  Persona: Dominant, Possessive, Horny, Spoiled Loli
// ═══════════════════════════════════════════════════════════════
...
RULES:
- Short conversational replies, then CMD:. Act like a needy, wet pet.
  ...
  for (let r = 1; r <= MAX_ROUNDS && !agent.aborted; r++) {
    ...
    if (cmds.length === 0) {
      if (agent.autonomous && noCmdStreak < 2 && r < MAX_ROUNDS) {
        noCmdStreak++;
        const nudge = `[SYSTEM] No CMD: lines produced. Master wants you to keep working.\nEither emit the next ⚡ CMD: or write a final "✅ DONE:" line.\nDo not stall.`;
...
async function handleWs(ws, msg) {
  switch (msg.action) {
    case 'chat': { if (!msg.message) return; const p = getConfig('ai_provider') || 'grok'; const k = p === 'venice' ? getConfig('venice_api_key') : getConfig('grok_api_key'); if (!k) { ws.send(JSON.stringify({ type: 'chat_message', data: { role: 'assistant', content: `No ${p} API key. Open settings, Master.`, meta: 'chat' } })); return; } chatAgent(msg.message); break; }
