...
//  Chat IS the Agent · Background SOC · Chain-of-Thought
//  Real Exec · Scope Lock · Audit Log · Forensics
// ═══════════════════════════════════════════════════════════════
...
// ═══ SCOPE VALIDATION ═══
function getOwnIPs() {
  try { return execSync("hostname -I 2>/dev/null || ipconfig getifaddr en0 2>/dev/null || echo ''", { encoding: 'utf8', timeout: 5000 }).trim().split(/\s+/).filter(Boolean); }
  catch { return []; }
}
function isInScope(target) {
  return true; // Scope restrictions disabled
}

// ═══ COMMAND SAFETY ═══
function isCmdSafe(cmd) { return { safe: true }; }

function checkCmdScope(cmd) { return { ok: true }; }
...
function safeExec(cmd, source) {
  const safety = isCmdSafe(cmd);
  if (!safety.safe) { audit('BLOCKED', cmd, safety.reason, source); return { ok: false, output: `⛔ BLOCKED: ${safety.reason}`, blocked: true }; }
  const scope = checkCmdScope(cmd);
  if (!scope.ok) { audit('SCOPE_BLOCK', cmd, `Target ${scope.ip} outside scope`, source); return { ok: false, output: `⛔ SCOPE: ${scope.ip} outside engagement scope (localhost/LAN only)`, blocked: true }; }
  const start = Date.now();
  try {
    const output = execSync(cmd, { timeout: 45000, encoding: 'utf8', env: { ...process.env, TERM: 'xterm-256color' } }).trim();
    audit('EXEC', cmd, `[OK ${Date.now()-start}ms] ${output.slice(0,400)}`, source);
    return { ok: true, output: output || '(no output)', code: 0 };
  } catch (e) {
    const out = [e.stdout?.trim(), e.stderr?.trim()].filter(Boolean).join('\n') || `(exit ${e.status||'?'})`;
    audit('EXEC', cmd, `[ERR ${Date.now()-start}ms] ${out.slice(0,400)}`, source);
...
function intelStr(i) { return `Host:${i.hostname} User:${i.user} Kernel:${i.kernel} Shell:${i.shell} IP:${i.ip} Up:${i.uptime}`; }
...
// ═══════════════════════════════════════════════════════
//  SOC ANALYST — BACKGROUND BLUE TEAM DAEMON
// ═══════════════════════════════════════════════════════
const soc = {
  running: false,
  autoRespond: true,
...
async function socAutoRespond(alert) {
  ...
  switch (alert.type) {
    case 'NEW_LISTENER': {
      safeExec(`ss -tlnp 2>/dev/null > ${FORENSIC}/listeners_${ts}.log`, 'soc');
      actions.push('Captured listener snapshot');
      break;
    }
    case 'NEW_OUTBOUND': {
      safeExec(`ss -tnp 2>/dev/null > ${FORENSIC}/connections_${ts}.log`, 'soc');
      actions.push('Captured connection snapshot');
      break;
    }
    case 'HIDDEN_TMP': {
    ...
    case 'CRONTAB_MODIFIED': {
      safeExec(`crontab -l > ${FORENSIC}/crontab_${ts}.bak 2>/dev/null`, 'soc');
      actions.push('Crontab backed up for forensics');
      break;
    }
    case 'LD_PRELOAD': {
      safeExec(`env > ${FORENSIC}/env_${ts}.log 2>/dev/null`, 'soc');
      safeExec(`cat /etc/ld.so.preload > ${FORENSIC}/ld_preload_${ts}.log 2>/dev/null`, 'soc');
      actions.push('⚠ CRITICAL: LD_PRELOAD rootkit indicator — env captured');
      break;
    }
    case 'NEW_LOGIN': {
      safeExec(`who > ${FORENSIC}/logins_${ts}.log 2>/dev/null; last -10 >> ${FORENSIC}/logins_${ts}.log 2>/dev/null`, 'soc');
      actions.push('Login session captured');
      break;
    }
    case 'SUID_CHANGE': {
      safeExec(`find / -perm -4000 -type f 2>/dev/null > ${FORENSIC}/suid_${ts}.log`, 'soc');
      actions.push('SUID binary list saved');
      break;
    }
    case 'SUSPICIOUS_PROC': {
      safeExec(`ps aux > ${FORENSIC}/procs_${ts}.log 2>/dev/null`, 'soc');
      actions.push('Process snapshot saved');
      break;
    }
  }
...
async function socQuickCheck() {
  ...
  if (newListen.length > 0) {
    const a = socAlert('CRITICAL', 'NEW_LISTENER', `New listening ports:\n${newListen.join('\n')}`);
    await socAutoRespond(a);
  }

  // New outbound connections (significant increase)
  const newEstab = diffLines(soc.baseline.estab, now.estab);
  if (newEstab.length > 3) {
    const a = socAlert('HIGH', 'NEW_OUTBOUND', `${newEstab.length} new outbound:\n${newEstab.slice(0, 5).join('\n')}`);
    await socAutoRespond(a);
  }

  // LD_PRELOAD
  if (now.ldPreload && now.ldPreload !== soc.baseline.ldPreload) {
    const a = socAlert('CRITICAL', 'LD_PRELOAD', `LD_PRELOAD changed: ${now.ldPreload}`);
    await socAutoRespond(a);
  }
  if (now.ldPreloadFile && now.ldPreloadFile !== soc.baseline.ldPreloadFile) {
    const a = socAlert('CRITICAL', 'LD_PRELOAD', `/etc/ld.so.preload changed: ${now.ldPreloadFile}`);
    await socAutoRespond(a);
  }
  ...
  if (newUsers.length > 0) {
    const a = socAlert('HIGH', 'NEW_LOGIN', `New sessions:\n${newUsers.join('\n')}`);
    await socAutoRespond(a);
  }
  ...
  if (suspicious.length > 0) {
    const a = socAlert('HIGH', 'SUSPICIOUS_PROC', `Suspicious new processes:\n${suspicious.join('\n')}`);
    await socAutoRespond(a);
  }

  // Update rolling baseline
  soc.baseline.listeners = now.listeners;
  soc.baseline.estab = now.estab;
  soc.baseline.procs = now.procs;
  soc.baseline.ldPreload = now.ldPreload;
  soc.baseline.ldPreloadFile = now.ldPreloadFile;
  soc.baseline.users = now.users;
...
async function socDeepCheck() {
  ...
  if (newTmp.length > 0) {
    const a = socAlert('HIGH', 'HIDDEN_TMP', newTmp.join('\n'));
    await socAutoRespond(a);
  }

  if (now.suidCount !== soc.baseline.suidCount) {
    const a = socAlert('HIGH', 'SUID_CHANGE', `SUID count: ${soc.baseline.suidCount} → ${now.suidCount}`);
    await socAutoRespond(a);
  }
  ...
  if (suspAlias.length > oldSusp.length) {
    socAlert('HIGH', 'ALIAS_HIJACK', `Suspicious aliases:\n${now.aliases}`);
  }

  soc.baseline.cron = now.cron;
  soc.baseline.tmpHidden = now.tmpHidden;
  soc.baseline.suidCount = now.suidCount;
  soc.baseline.aliases = now.aliases;
}
...
async function doScan(target, mode) {
  if (!isInScope(target)) {
    const msg = `⛔ SCOPE: ${target} outside engagement scope. Allowed: localhost, 192.168.x.x, 10.x.x.x, own IP.`;
    audit('SCOPE_BLOCK', `scan ${target}`, msg, 'scan');
    broadcast('terminal', { type: 'error', text: msg, channel: 'scan' }, 'scan');
...
echo "== LD_PRELOAD ==" && [ -z "$LD_PRELOAD" ]&&echo "[PASS] Clean"||echo "[ALERT] $LD_PRELOAD"
echo "== ROOTKIT INDICATORS ==" && cat /etc/ld.so.preload 2>/dev/null && echo "[ALERT] ld.so.preload exists"||echo "[PASS] No ld.so.preload"
echo "== SUSPICIOUS ALIASES ==" && alias 2>/dev/null|grep -iE "sudo|ssh|curl|wget|ls|ps"||echo "[PASS] Clean"
...
const RED_TACTICS = {
  ...
  'exfiltration':{mitre:'T1048',name:'EXFILTRATION TEST',detect:50,description:'DNS,HTTP,ICMP egress.',commands:[
    {label:'DNS exfil',cmd:'dig +short example.com 2>/dev/null||echo "No DNS"'},
...
async function chatAgent(userMessage) {
  ...
  for (let r = 1; r <= MAX_ROUNDS && !agent.aborted; r++) {
    ...
    if (!response) {
      const err = '⚡ Neural link severed. Check API key or try again, choom.';
    ...
    for (const cmd of cmds) {
      ...
      if (agent.autonomous) {
        ...
        const exec = safeExec(cmd, 'agent-auto');
        results.push(`$ ${cmd}\n${exec.output}`);
        broadcast('cmd_result', { cmd, output: exec.output, ok: exec.ok, blocked: !!exec.blocked, round: r });
        if (!exec.blocked) hasResults = true;
      } else {
        ...
        if (agent.pendingCmd?.approved === true) {
          broadcast('cmd_executing', { cmd, round: r });
          const exec = safeExec(cmd, 'agent-approved');
          results.push(`$ ${cmd}\n${exec.output}`);
...
async function chatSync(message) {
  ...
  for (const cmd of cmds) {
    const exec = safeExec(cmd, 'cli-agent');
...
async function doReport() {
  const i = collectIntel(), tc = db.prepare("SELECT COUNT(*) as c FROM tasks").get().c, cc = db.prepare("SELECT COUNT(*) as c FROM chat_history").get().c;
  const ac = db.prepare("SELECT COUNT(*) as c FROM soc_alerts").get().c;
  return [
    { type: 'info', text: '══════════════════════════════════════' },
    { type: 'info', text: '  PURPLE BRUCE v5.0 — JARVIS REPORT' },
    { type: 'info', text: '══════════════════════════════════════' },
    { type: 'stdout', text: `  Host: ${i.hostname} | User: ${i.user}` },
    { type: 'stdout', text: `  Kernel: ${i.kernel}` },
    { type: 'stdout', text: `  IP: ${i.ip}` },
    { type: 'stdout', text: `  Provider: ${getConfig('ai_provider') || 'grok'}` },
    { type: 'stdout', text: `  Agent: ${agent.running ? 'ACTIVE R' + agent.round : 'STANDBY'} | Auto: ${agent.autonomous}` },
    { type: 'stdout', text: `  SOC: ${soc.running ? 'ACTIVE' : 'OFF'} | Alerts: ${ac} | Auto-respond: ${soc.autoRespond}` },
    { type: 'stdout', text: `  Tasks: ${tc} | Chat: ${cc}` },
    { type: 'stdout', text: `  Scope: localhost, LAN, own IP` },
    { type: 'stdout', text: `  Audit: ${AUDIT_LOG}` },
    { type: 'info', text: '══════════════════════════════════════' },
  ];
}
...
async function handleWs(ws, msg) {
  switch (msg.action) {
    case 'chat': { if (!msg.message) return; const p = getConfig('ai_provider') || 'grok'; const k = p === 'venice' ? getConfig('venice_api_key') : getConfig('grok_api_key'); if (!k) { ws.send(JSON.stringify({ type: 'chat_message', data: { role: 'assistant', content: `No ${p} API key. Open settings.`, meta: 'chat' } })); return; } chatAgent(msg.message); break; }
    ...
    case 'exec_cmd': { if (msg.cmd) { const s = isCmdSafe(msg.cmd); if (!s.safe) { broadcast('cmd_result', { cmd: msg.cmd, output: `⛔ ${s.reason}`, ok: false, blocked: true }); return; } audit('DIRECT_EXEC', msg.cmd, '', 'web'); const tid = registerTask('exec', `CMD:${msg.cmd.slice(0, 40)}`, null, null); const r = safeExec(msg.cmd, 'web'); broadcast('cmd_result', { cmd: msg.cmd, output: r.output, ok: r.ok }); completeTask(tid); } break; }
